...
CROSS JOIN (
  SELECT
    financniZdrojKod AS type
  FROM `pomocne.source_type_lookup` 
  WHERE financniZdrojKod IN ('t1','t2','t7','z2')
) AS T
...
LEFT JOIN (
    ...
    GROUP BY organization_id,year,type
) AS G ON G.organization_id = M.id AND G.year = M.year AND G.type = t.type
